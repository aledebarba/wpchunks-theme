...
class Components_Core_Options {
    ...
    public function addComponent($fnName, $path, $url="") {
        ...
        $this->componentStore[$name] = ['function'=>$fnName, 'path' => $path, 'url' => $url, 'instance' => 0];
            ...
            if(isset($package->wpchunk)) {
                ...
                if($type == "react_app") {
                    $fnBody = "function $fnName(){ 
                        ...
                        return call_user_func( 
                            'Component::jsbundle', 
                            '$name', func_get_args(), ['wp-element', 'wp-editor'], true, true ); 
                        }";
                    // eval function
                    eval($fnBody);
                }
...
class Import {
    ...
    function from($folder) {
        global $components_core_options;
        if($folder !== "") {
            $this->path = get_stylesheet_directory()."/$folder";
            $this->url =  get_stylesheet_directory_uri()."/$folder";
        } else {
            $this->path = get_stylesheet_directory()."/{$this->defaultPath}/$folder";
        }
        $components_core_options->addComponent($this->functionName, $this->path, $this->url);
    }
